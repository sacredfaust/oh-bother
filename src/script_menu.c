...
static u8 sVermilionSSTidalSelections[SSTIDAL_SELECTION_COUNT];
...
static void CreateVermilionSSTidalMultichoice(void)
{
    ...
    if (gSpecialVar_0x8004 == 0)
    {
        sVermilionSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_SLATEPORT;
        selectionCount++;
        sVermilionSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_LILYCOVE;
        selectionCount++;
        sVermilionSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_SEVII;
        selectionCount++;

        if (FlagGet(FLAG_MET_SCOTT_ON_SS_TIDAL) == TRUE)
        {
            sVermilionSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_BATTLE_FRONTIER;
            selectionCount++;
        }
    }
    ...
    if (CheckBagHasItem(ITEM_EON_TICKET, 1) == TRUE && FlagGet(FLAG_ENABLE_SHIP_SOUTHERN_ISLAND) == TRUE)
    {
        if (gSpecialVar_0x8004 == 0)
        {
            sVermilionSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_SOUTHERN_ISLAND;
            selectionCount++;
        }
    ...
    if (CheckBagHasItem(ITEM_MYSTIC_TICKET, 1) == TRUE && FlagGet(FLAG_ENABLE_SHIP_NAVEL_ROCK) == TRUE)
    {
        if (gSpecialVar_0x8004 == 0)
        {
            sVermilionSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_NAVEL_ROCK;
            selectionCount++;
        }
    ...
    if (CheckBagHasItem(ITEM_AURORA_TICKET, 1) == TRUE && FlagGet(FLAG_ENABLE_SHIP_BIRTH_ISLAND) == TRUE)
    {
        if (gSpecialVar_0x8004 == 0)
        {
            sVermilionSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_BIRTH_ISLAND;
            selectionCount++;
        }
    ...
    if (CheckBagHasItem(ITEM_OLD_SEA_MAP, 1) == TRUE && FlagGet(FLAG_ENABLE_SHIP_FARAWAY_ISLAND) == TRUE)
    {
        if (gSpecialVar_0x8004 == 0)
        {
            sVermilionSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_FARAWAY_ISLAND;
            selectionCount++;
        }
    ...
    sVermilionSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_EXIT;
    selectionCount++;
...
static void CreateSeviiSSTidalMultichoice(void)
{
    ...
    else
    {
        ...
        for (j = 0; j < SSTIDAL_SELECTION_COUNT_S; j++)
        {
            u8 selection = sSeviiSSTidalSelections[j];
            if (selection != 0xFF)
            {
                pixelWidth = DisplayTextAndGetWidth(sSeviiSSTidalDestinations[selection], pixelWidth);
            }
        }
        ...
        for (selectionCount = 0, i = 0; i < SSTIDAL_SELECTION_COUNT_S; i++)
        {
            if (sSeviiSSTidalSelections[i] != 0xFF)
            {
                AddTextPrinterParameterized(windowId, 1, sSeviiSSTidalDestinations[sSeviiSSTidalSelections[i]], 8, selectionCount * 16 + 1, TEXT_SKIP_DRAW, NULL);
                selectionCount++;
            }
        }
